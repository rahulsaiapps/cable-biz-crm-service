...
public class AuthController {
    ...
    public ResponseEntity<Map<String, Object>> tokenSwap(@RequestBody TokenSwapRequest request) {
        ...
        try {
            ...
            try {
                ...
                logger.warn(
                        "Token-swap reconciliation degraded for uid={}: {}",
                        uid,
            ...
            logger.error("Token-swap failed: {}", e.getMessage(), e);
            return ResponseEntity.status(400).body(errorResponse("Token validation failed: " + e.getMessage()));
        }
    }
    ...
    private Map<String, Object> buildTokenResponse(
        ...
        try {
            ...
            if (firebaseFallbackToken == null || firebaseFallbackToken.isBlank()) {
                throw jwtError;
            }
            logger.warn(
                    "JWT issuance failed for uid={}; falling back to Firebase token echo: {}",
                    uid,
                    jwtError.getMessage());
